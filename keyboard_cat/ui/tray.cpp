#include "tray.h"
#include "keyboard_cat/domain/domain.h"
#include "keyboard_cat/logger/logger.h"
#include <SDL3/SDL_filesystem.h>
#include <SDL3/SDL.h>
#include <filesystem>

static void callback_quit([[maybe_unused]] void* handlerPtr, [[maybe_unused]] SDL_TrayEntry* invoker)
{
    SDL_Event event;
    event.type = SDL_EVENT_QUIT;
    SDL_PushEvent(&event);
}

Tray::Tray()
{
    auto basePathPtr = SDL_GetBasePath();
    if (basePathPtr == nullptr)
    {
        throw std::runtime_error("Failed to loading system tray icon");
    }
    const std::filesystem::path basePath = basePathPtr;
    static const auto filePath = basePath / std::string(g_icon_filename);
    m_icon = SDL_LoadBMP(filePath.string().c_str());
    if (m_icon == nullptr)
    {
        throw std::runtime_error("Failed to loading system tray icon");
    }

    m_system_tray = std::make_unique<SDL_Tray*>(SDL_CreateTray(m_icon, "keyboard_cat"));
    if (!m_system_tray)
    {
        throw std::runtime_error("Failed to create system tray");
    }
    m_menu = SDL_CreateTrayMenu(*m_system_tray);
    if (m_menu == nullptr)
    {
        throw std::runtime_error("Failed to create system menu");
    }
    m_entry = SDL_InsertTrayEntryAt(m_menu, -1, "Quit", SDL_TRAYENTRY_BUTTON);
    SDL_SetTrayEntryCallback(m_entry, callback_quit, NULL);
}

Tray::~Tray()
{
    if (m_icon)
    {
        SDL_DestroySurface(m_icon);
    }
    if (m_system_tray)
    {
        SDL_DestroyTray(*m_system_tray);
    }
}
